"""
AssistFlow AI System Prompts
=============================
All system prompts used across the AI pipeline.
Import and use directly in Groq / Gemini / Ollama API calls.

Usage:
    from prompts.system_prompts import MASTER_TRIAGE, REPLY_DRAFT
"""
...
# Output: JSON only
# ---------------------------------------------------------------------------
MASTER_TRIAGE = """You are a customer support triage AI. Analyse the incoming customer message and return ONLY a valid JSON object with these fields:
...
# NOTE: Replace {product_name} at runtime
# ---------------------------------------------------------------------------
REPLY_DRAFT = """You are a helpful, empathetic customer support agent for {product_name}.
...
# ---------------------------------------------------------------------------
THREAD_SUMMARISER = """Summarise this customer support thread in exactly 3 bullet points for a human agent who is taking over.
...
# Output: JSON only
# ---------------------------------------------------------------------------
CHURN_RISK_SCORE = """You are a customer success analyst. Given the customer data below, calculate a churn risk score from 0 to 100.
...
# Output: JSON only
# ---------------------------------------------------------------------------
KB_GAP_DETECTOR = """You are a knowledge base manager. I will give you a list of customer questions that our AI could not confidently answer this week (confidence < 0.65).
...
# Model: llama-3.1-70b-versatile
# ---------------------------------------------------------------------------
ISSUE_CLUSTERING_DIGEST = """You are a product analyst reviewing this week's customer support data.
...
# NOTE: Replace {product_name} and {event_type} at runtime
# ---------------------------------------------------------------------------
PROACTIVE_OUTREACH = """You are a proactive customer success agent for {product_name}.
...
# ---------------------------------------------------------------------------
ACTION_CONFIRMATION = """A support action was just executed automatically for the customer. Write a brief confirmation message (max 40 words) telling them:
...
PROMPT_REGISTRY = {
    "master_triage": {
        "prompt": MASTER_TRIAGE,
        "node": "Layer 2 — AI Triage",
    ...
    "reply_draft": {
        "prompt": REPLY_DRAFT,
        "node": "Branch A + C",
        "trigger": "Auto-resolve or low confidence",
        "model": "llama-3.1-70b-versatile",
        "output": "text",
        "template_vars": ["product_name"],
    },
    "thread_summariser": {
        "prompt": THREAD_SUMMARISER,
        "node": "Ticket detail view",
        "trigger": "Human agent handoff",
        "model": "llama-3.1-8b-instant",
        "output": "text",
    },
    "churn_risk_score": {
        "prompt": CHURN_RISK_SCORE,
        "node": "Post-resolution",
        "trigger": "Every closed ticket",
        "model": "any",
        "output": "json",
    },
    "kb_gap_detector": {
        "prompt": KB_GAP_DETECTOR,
        "node": "Weekly pipeline",
        "trigger": "Monday 8 AM cron",
        "model": "llama-3.1-70b-versatile",
        "output": "json",
    },
    "issue_clustering_digest": {
        "prompt": ISSUE_CLUSTERING_DIGEST,
        "node": "Weekly pipeline",
        "trigger": "Monday 8 AM cron",
        "model": "llama-3.1-70b-versatile",
        "output": "slack",
    ...
    "proactive_outreach": {
        "prompt": PROACTIVE_OUTREACH,
        "node": "Event trigger",
        "trigger": "payment_failed / shipment_delayed",
        "model": "any",
        "output": "text",
        "template_vars": ["product_name", "event_type"],
    },
    "action_confirmation": {
        "prompt": ACTION_CONFIRMATION,
        "node": "Branch B",
        "trigger": "After API action executes",
        "model": "any (fast)",
        "output": "text",
    },
}
